...
3. **Strip the exercise material from the clone — `_solutions/`, `README.md`, `practice.json`.**
   ...
   the service repo plus the work items** — the ticket, the feature request, the source, the
   tests, the stack manifest — so the assistant meets the task the way an engineer would.
