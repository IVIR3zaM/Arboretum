...
| `practices/<id>/_solutions/` | The answer key, hidden from the learner: hidden grader/acceptance tests, `grade.*`, `trap-manifest.md`, `rubric.md`, `feature-qa.md`, `FIX.md`, `FEATURE-FIX.md` + `feature-reference/` (a reference elicited build of the phase-3 feature), **proof files** (`proof-<mode>-<YYYY-MM-DD>.html`), `misbehaviors.md` (the running inventory of what a recorded run exposed — assistant behaviour, mis-calibrated traps, harness gaps — used to plan improvements), and Cedar's `context-map.md` / `doc-drift-ledger.md`. | — |
