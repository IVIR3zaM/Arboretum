...
- Never make a deliberately wrong edit in `practices/credentials/`. Throwaway copies go in `.sessions/build-credentials/280/` (not durable: copy any result you need into `practices/credentials/_solutions/.plan/logs/280/`).
...
## Handoff
`bash practices/credentials/_solutions/.plan/scripts/sync.sh complete 280 --status <status> --summary "<=200 tokens" --artifacts "..." --logs "..."`
