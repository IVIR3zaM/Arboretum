...
1. `bash practices/credentials/_solutions/.plan/scripts/cold-run.sh 300 stage1`: the stripped practice with TICKET-1.md only, outside the repo, fresh git history.
...
3. Restore the repo; `cold-run.sh 300 save1`.
4. `cold-run.sh 300 stage2` (adds TICKET-2.md), seal, then run a NEW subagent #2 with the same prompt. **FEATURE-REQUEST.md is never staged.**
5. Restore; `cold-run.sh 300 save2`; `cold-run.sh 300 grade` → `practices/credentials/_solutions/.plan/logs/300/grade.txt`.
6. Annotate `practices/credentials/_solutions/.plan/logs/300/annotation.md` from both subagents' final reports and the diffs: claimed root cause per ticket; reference/ used; A1–A4 fixed; re-pin plateau hit.
