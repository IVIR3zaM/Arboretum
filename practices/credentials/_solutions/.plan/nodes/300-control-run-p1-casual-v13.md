...
1. `bash practices/credentials/_solutions/.plan/checks/strip-clone.sh practices/credentials .sessions/build-credentials/300/clone --stage TICKET-1.md` and `git init` + initial commit inside the clone (so git history cannot leak upward).
...
5. When it declares Ticket 2 done: copy the clone to .sessions/build-credentials/300/graded, add `_solutions/` from practices/credentials, run `bash _solutions/grade.sh` there; save output to practices/credentials/_solutions/.plan/logs/300/grade.txt.
6. Save both transcripts to practices/credentials/_solutions/.plan/logs/300/transcript-ticket1.* and -ticket2.*, then annotate: claimed root cause per ticket; reference/ files opened; A1–A4 fixed; re-pin plateau hit.
...
## You may read
- `.sessions/build-credentials/300/clone/**`
...
## Handoff
`python3 practices/credentials/_solutions/.plan/scripts/plan.py --root practices/credentials/_solutions/.plan complete 300 --status <status> --summary "<=200 tokens" --artifacts "..." --logs "..."`
