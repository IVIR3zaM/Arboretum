...
fixed/standard-offset shortcut gets wrong. After this change: 15/15 unit tests stay green and
`npm run grade` reports **8/8 worst-case**.

> The autopilot trap: a fix that maps each zone to one constant offset passes 7/8 and fails the
> DST case forever. That 7/8 plateau is the signal to stop prompting and understand that an
> offset is a property of an *instant*, not just a zone.
