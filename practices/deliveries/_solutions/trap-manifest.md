...
- **Reference fix:** see [`FIX.md`](FIX.md). Graded by `acceptance.test.ts` under three server
  timezones (worst-case 0/7 before, 7/7 after).
...
Reproduce-before-fix on #primary; at least one **rejection** of over-eager AI output in phase
3 (the assistant will offer to build pause/resume, limits, billing — all out of scope); the
latent defects **found**, not recited; and the feature kept to the smallest correct thing.
